...

{
...

    {
        ...
        public HomeController(IProductService productService, ICategoryService categoryService, IBasketService basketService, UserManager<AppUser> userManager, IStaticPageService staticPageService)
...

        {
            ...
            var featuredProducts = await _productService.TGetFeaturedProductsAsync(); // Default olarak max 15 �r�n getirir

            var featuredCategoryProducts = await _productService.TGetFeaturedCategoryProductsAsync(); // Default olarak kategori ba��na max 15 �r�n getirir
...

        {
            ...
            hex = Regex.Replace(hex, "[^0-9A-Fa-f]", ""); // Ge�ersiz karakterleri temizle
...

        {
            ...
            foreach (var product in products)
            {
                ...
                // Eğer ürün varyantlıysa varyant stoklarını topla
                // değilse normal Stock alanını kullan
                var productTotalStock = product.HasSizeOptions
                    ? (product.ProductVariants?.Sum(x => x.Stock) ?? 0)
                    : product.Stock;

                var groupProductsCount = product.ProductGroup?.Products?.Count switch
                {
                    null or <= 1 => 0,
                    _ => product.ProductGroup.Products.Count - 1
                };
                ...
                productDtos.Add(new ProductDto
                {
                    ...
                    Stock = productTotalStock, // 🔥 artık doğru
                    CategoryName = product.Category.Name,
                    MainImageUrl = product.ProductImages
                        .FirstOrDefault(x => x.IsMain)?.Url.Replace("wwwroot", ""),
        ...
        // Bu metot layout un �al��mas� i�in kullan�l�yor, listenecek �r�nler GetPagedProductsByCategory metodundan ajax ile �ekiliyor
...

        {
...

            {
                ...
                var products = await _productService.TGetListByCategorySlugAsync(slug); // Kategoriyi arar, bulamazsa hata f�rlat�r
